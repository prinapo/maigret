...
describe("BookDetails.vue admin", () => {
  it("mostra tutti i campi e la sezione extra se admin", async () => {
  ...
  it("mostra tabella per array di oggetti", async () => {
    const wrapper = mountWithStores();
    await flushPromises();
    expect(wrapper.find(".q-table").exists()).toBe(true);
  });

  it("mostra chip per array di valori semplici", async () => {
    const book = { ...bookMock, tags: ["uno", "due"] };
    const wrapper = mountWithStores(book);
    await flushPromises();
    expect(wrapper.findAll(".q-chip").length).toBeGreaterThan(0);
  });

  it("mostra toggle per boolean se editabile", async () => {
  ...
  it("mostra select per campo annoPubblicazione con anni dal 1920", async () => {
    const wrapper = mountWithStores();
    await flushPromises();
    const selects = wrapper.findAll(".q-select");
    // Cerca select con label "Anno Pubblicazione" (puoi raffinare se serve)
    expect(selects.length).toBeGreaterThan(0);
  });

  it("feedback inline su salvataggio campo", async () => {
    // Simula stato di salvataggio
    expect(true).toBe(true); // Placeholder
  });

  it("non salva campi extra su Firebase", async () => {
    expect(true).toBe(true); // Placeholder
  });
});
