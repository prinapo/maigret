...
async function getDeviceSerial() {
  ...
  const devices = execSync("adb devices")
    .toString()
    .split("\n")
    .filter(line => line.match(/\bdevice\b/) && !line.startsWith("List"))
    .map(line => line.trim().split(/\s+/)[0]);
  console.log("DEBUG: Device trovati da adb:", devices);
  if (!deviceSerial) {
    if (devices.length === 0) {
      throw new Error("Nessun device trovato! Collega almeno un dispositivo via USB o imposta DEVICE_SERIAL.");
      ...
      while (idx < 0 || idx >= devices.length) {
        const answer = await ask(`Scegli il dispositivo su cui installare l'app [1-${devices.length}]: `);
