...
const columns = 4; // Numero di colonne della griglia
...
const getImageSource = computed(() => (imageName) => {
  if (imageSourceCache.has(imageName)) {
    return imageSourceCache.get(imageName);
  }
  ...
  if (!imageName || imageName === "placeholder.jpg") {
    ...
    source = `${fireStoreTmblUrl}${encodeURIComponent(imageName)}?alt=media`;
  }

  imageSourceCache.set(imageName, source);
  return source;
});
...
const getGridItems = computed(() => {
  const filteredBooks = sortedBibliografia.value.filter(shouldShowBook);
  const gridItems = [];
  for (let i = 0; i < filteredBooks.length; i += columns) {
    gridItems.push(filteredBooks.slice(i, i + columns));
  }
  return gridItems;
});
...
.virtual-scroller {
  height: 80vh; /* Imposta l'altezza del contenitore del virtual scroller */
}
